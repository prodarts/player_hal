// 画像ギャラリーの横スクロール機能 および JSONデータ反映処理
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('https://prodarts.github.io/player_hal/player_hal.json') // ★ここを絶対パスに修正★
    ...
    .then(data => {
      ...
      document.getElementById('player-image').src = `https://prodarts.github.io/player_hal/${data.profile.profileImage}`; // ★画像パスも修正★
      ...
      document.querySelector('#player-instagram img').src = `https://prodarts.github.io/player_hal/${data.sns.instagram.icon}`; // ★アイコンパスも修正★
      document.querySelector('#player-x img').src = `https://prodarts.github.io/player_hal/${data.sns.x.icon}`; // ★アイコンパスも修正★
      ...
      data.gallery.forEach(item => {
        ...
        // 画像パスも必要に応じて絶対パスまたはルートからの相対パスに修正
        div.innerHTML = `<img src="https://prodarts.github.io/player_hal/${item.src}" alt="${item.alt}" title="${item.title || item.alt}">`; // ★画像パスも修正★
      ...
      if (data.achievements.testimonial && data.achievements.testimonial.comment) {
      ...
      if (jsonLdScript) {
          const jsonLdData = JSON.parse(jsonLdScript.textContent); // 既存のJSON-LDをパース
          jsonLdData.name = data.profile.nameJP;
          jsonLdData.alternateName = data.profile.nameEN;
          jsonLdData.image = data.meta.og.image;
          jsonLdData.description = data.meta.description + (data.profile.nameEN ? ` ${data.profile.nameEN} is a professional darts player based in ${data.profile.pref}, Japan.` : '');
          jsonLdData.url = data.meta.og.url;
          
          jsonLdData.sameAs = [data.sns.instagram.url, data.sns.x.url];
          
          if (Array.isArray(data.profile.org)) { // orgが配列であることを確認
              jsonLdData.memberOf = data.profile.org.map(orgName => ({
                  "@type": "SportsOrganization",
                  "name": orgName
              }));
          } else { // 配列でない場合は既存の形式を考慮
              jsonLdData.memberOf = [{ "@type": "SportsOrganization", "name": data.profile.org }];
          }
          ...
          // achievements.titlesが配列であることを確認し、文字列の配列として設定
          jsonLdData.award = data.achievements.titles && Array.isArray(data.achievements.titles) 
                            ? data.achievements.titles.map(item => item.replace(/<br>/g, '')) // <br>タグを除去
                            : [];
          
          jsonLdData.alumniOf = data.profile.subcatch.split('\n')[0]; // サブキャッチの最初の行を例として使用
    ...
    .catch(err => {
      ...
      if (mainContainer) {
          mainContainer.innerHTML = '<p style="text-align:center; color:red; margin-top:50px;">コンテンツの読み込みに失敗しました。ファイルパスやJSONデータをご確認ください。</p>';
      }
    });
});
